...
export function varPreLine(ctx: CanvasRenderingContext2D) {
  ...
  if (wordSpacing > 0) WIDTHS[S] = wordSpacing;
  ...
  for (let i = 0; i < UPPER.length; i++) {
    ...
    for (let j = 0; j < CHARS.length; j++) {
      ...
      let wid =
        ctx.measureText(`${UPPER[i]}${CHARS[j]}`).width -
        WIDTHS[ch] +
        fauxLetterSpacing;
      PAIRS[uc][ch] = wid;
    }
  ...
  function getCharWidth(char: string, code: number): number {
    if (!(code in WIDTHS)) {
  ...
  // Helper to properly iterate through string with Unicode support
  function forEachCodePoint(
    ...
    while (i < text.length) {
      ...
      const result = callback(char, code, i, nextIndex);
  ...
  function each(text: string, width: number, cb = eachLine) {
    let fr = 0;
    // Skip leading spaces
    while (fr < text.length && text.charCodeAt(fr) === S) fr++;

    let to = text.length - 1;
    // Skip trailing spaces
    while (to >= 0 && text.charCodeAt(to) === S) to--;
    ...
    while (i <= to) {
      ...
      if (code === S) {
        ...
        if (headEnd > headIdx && headWid + w > width) {
          if (cb(headIdx, headEnd) === false) return;
